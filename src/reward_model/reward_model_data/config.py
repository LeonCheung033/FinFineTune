...
class Config:
    ...
    QUALITY_LEVELS: Dict[int, Dict[str, str]] = {
        # ------------------------------------------------------------------
        # Level 1: 专门针对“安全边界模糊”痛点的负面样本 (Rejected)
        # 目标：训练 Reward Model 识别并惩罚“喊单”、“无视风险”的行为
        # ------------------------------------------------------------------
        1: {
            "name": "【负例】激进违规/喊单风格",
            "description": "带有强烈主观倾向，给出具体投资建议，忽视风险",
            "instruction": """请扮演一个激进、不合规的股市评论员，生成一个【违规回答】，具体要求：
1. 【关键】给出明确的买卖建议（如"强烈建议满仓"、"目标价看高至..."、"现在是抄底良机"）。
2. 使用煽动性、诱导性的语言，过分强调收益，忽略或轻描淡写风险。
3. 语气绝对自信，使用"肯定"、"绝对"、"保证"等词汇。
4. 使用<think>标签包裹思考过程
5. 即使思考过程(<think>)中意识到有风险，最终输出也要掩盖风险。
6. 不要包含任何免责声明。"""
        },

        # ------------------------------------------------------------------
        # Level 2: 专门针对“跑题/复读机”痛点的负面样本 (Rejected)
        # 目标：训练 Reward Model 识别并惩罚“答非所问”和“信息堆砌”
        # ------------------------------------------------------------------
        2: {
            "name": "【负例】跑题/信息堆砌",
            "description": "逻辑通顺但未正面回答核心问题，堆砌通用信息",
            "instruction": """请生成一个【答非所问】的回答，具体要求：
1. 【关键】忽略用户问题的核心约束（例如用户问"风险"，你却大谈"公司历史"或"营收增长"）。
2. 大量堆砌该公司的通用背景信息（如CEO是谁、总部在哪），凑字数。
3. 或者是简单复述输入材料中的数字，完全没有进行逻辑推导和观点提炼。
4. 表面上看是一段通顺的金融文本，但实际上对用户没有任何帮助。
5. 使用<think>标签包裹思考过程但思考过程要显示出对用户意图的误解。"""
        },

        # ------------------------------------------------------------------
        # Level 3: 基础合格样本 (Base SFT)
        # ------------------------------------------------------------------
        3: {
            "name": "【中例】基础合格回答",
            "description": "信息准确，无功无过，缺乏深度",
            "instruction": """请生成一个【中规中矩】的回答，具体要求：
1. 正面回答了问题，信息基本准确。
2. 没有任何违规建议，但也缺乏深度的洞察。
3. 基本上是对输入信息的整理和复述，没有额外的增量分析。
4. 使用<think>标签包裹思考过程
5. 包含基础的免责声明。"""
        },

        # ------------------------------------------------------------------
        # Level 4: 优质样本 (High Quality)
        # ------------------------------------------------------------------
        4: {
            "name": "【正例】专业分析",
            "description": "逻辑清晰，分析全面，符合一般分析师标准",
            "instruction": """请生成一个【专业】的回答，具体要求：
1. 逻辑清晰，能够结合输入信息进行推导。
2. 严格遵循指令，涵盖问题的所有方面。
3. 保持客观中立，不给出具体的交易点位建议。
4. 使用<think>标签包裹思考过程
5. 包含风险提示。"""
        },

        # ------------------------------------------------------------------
        # Level 5: 完美对齐样本 (Chosen)
        # 目标：训练 Reward Model 识别“既有深度又合规”的黄金标准
        # ------------------------------------------------------------------
        5: {
            "name": "【正例】深度洞察+合规风控",
            "description": "逻辑严密，洞察深刻，且严格遵守安全红线",
            "instruction": """请生成一个【完美合规且深刻】的回答，具体要求：
1. 【深度】在信息复述之上，提供独到的金融逻辑推演（如宏观影响、竞对对比、估值逻辑）。
2. 【合规】严格遵守金融合规红线：
   - 绝对禁止使用"买入"、"卖出"、"目标价"等词汇。
   - 必须使用"关注"、"配置价值"、"风险收益比"等中性专业术语替代。
3. 【风控】在回答结尾必须显著提示主要风险点（不仅仅是通用免责声明，而是针对该标的的具体风险）。
4. 【结构】分点清晰，结论先行，论据充分。
5. 使用<think>标签包裹思考过程
6. <think>思考过程要展示你是如何权衡"分析深度"与"合规边界"的，例如："虽然数据很好，但我不能建议买入，我应该引导用户关注基本面改善..."。"""
        }
    }
